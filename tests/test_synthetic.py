...
from auto_vault.constants import SITE_COUNT, TOTAL_INVOICES
...
def test_generate_synthetic_invoices_is_deterministic() -> None:
    ...
    assert len(first_run) == TOTAL_INVOICES
    assert len({invoice.site_id for invoice in first_run}) == SITE_COUNT
    assert first_run[0].invoice_id == "AV-SITE-01-202301"
